...
>
> 你不需要会编程，也不用手敲命令——只要在对话里用**日常语言描述你的试验设计**，就能完成 **49 种**专业的样本量与检验效能计算。默认权威引擎是**远程 coze R 计算服务**（rpact / gsDesign / TrialSize / PowerTOST 等 20+ 包在服务器端运行，本机**无需安装 R**）；另有 5 种基础检验的纯 Python 兜底可离线使用。结果默认按操作系统语言设定给出中文或英文（可用提示词强制切换）。技能默认展示**安全预览**——即它将发往 coze 的精确请求信封，在你确认前不会真正发出；完整 R 代码可按需返回。
...
- **安全预览 / coze 计算是什么**：发布版技能**绝不在你本机运行 R 或 shell**。默认它调用远程 **coze** 计算服务（端点：`https://ct-samplesize.coze.site/run`），只发送试验设计参数（绝不发送患者数据）。想先看一眼，说 **「仅预览 / --dry-run」**——会打印精确的请求信封、不发任何东西；说 **「请直接计算」**（或 **--yes**）即发送并取回数字与可选图形；说 **「展示代码 / --show-code」** 查看 coze 请求 JSON（含按需返回的 R 源码）。
- **仅 coze（v5）**：发布版技能**没有本地计算兜底**——coze 端点不可达时会提示配置 `CTSS_COZE_ENDPOINT`（或 `CTSS_COZE_MOCK=1` 本地演示），不会静默回退；全部 49 种检验均经 coze 服务端计算。
- **输出仅供参考**，用于监管申报前请独立验证。
